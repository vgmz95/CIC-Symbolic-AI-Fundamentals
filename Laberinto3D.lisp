        ...
        (cond 
            ((eql etiqueta-op :Arriba) 
                ...
                (if (setq not-out-of-bounds  (NOT (out-of-bounds? new-x new-y))) 
                    (cond 
                        ((eql (get-cell-walls x y) 16) (setq invalid-movement (= level 1)))
                        ((eql (get-cell-walls x y) 17) (setq invalid-movement (= level 0)))
                        (T   (setq invalid-movement (theres-wall? x y 0)))
                    ) 
                )
            )
            ((eql etiqueta-op :Derecha) 
                ...
                (if (setq not-out-of-bounds  (NOT (out-of-bounds? new-x new-y))) 
                    (cond 
                        ((eql (get-cell-walls x y) 16) (setq invalid-movement (= level 0)))
                        ((eql (get-cell-walls x y) 17) (setq invalid-movement (= level 1)))
                        (T   (setq invalid-movement (theres-wall? x y 1)))
                    )           
                )                  
            )
            ((eql etiqueta-op :Abajo) 
                ...
                (if (setq not-out-of-bounds  (NOT (out-of-bounds? new-x new-y))) 
                    (cond 
                        ((eql (get-cell-walls x y) 16) (setq invalid-movement (= level 1)))
                        ((eql (get-cell-walls x y) 17) (setq invalid-movement (= level 0)))
                        (T   (setq invalid-movement (theres-wall? x y 2)))
                    ) 
                )                
            )
            ((eql etiqueta-op :Izquierda) 
                ...
                (if (setq not-out-of-bounds  (NOT (out-of-bounds? new-x new-y))) 
                    (cond 
                        ((eql (get-cell-walls x y) 16) (setq invalid-movement (= level 0)))
                        ((eql (get-cell-walls x y) 17) (setq invalid-movement (= level 1)))
                        (T   (setq invalid-movement (theres-wall? x y 3)))
                    ) 
                )                
            )
        )
...
(defun  apply-operator (op  estado) 
    ...
    (let* ( (x      (aref  estado 0))                         
            (y      (aref  estado 1))
            (level  (aref  estado 2))
            (nuevo-estado  (make-array 3))
            (etiqueta-op (first op))
            (current-wall (get-cell-walls x y))
            (new-wall 0)
        )
        (cond 
            ((eql etiqueta-op :Arriba)      (setf (aref  nuevo-estado 0) (- x 1) (aref  nuevo-estado 1) y) )
        ...
        (setq new-wall (get-cell-walls (aref  nuevo-estado 0) (aref  nuevo-estado 1) ) )
        ;es-bajo puente? es puente?
        (cond 
            ((AND (OR (eql etiqueta-op :Arriba) (eql etiqueta-op :Abajo))       ( =  new-wall 17) )     (setf (aref  nuevo-estado 2) 1))     ;Entra a un puente
            ((AND (OR (eql etiqueta-op :Arriba) (eql etiqueta-op :Abajo))       ( =  current-wall 17) ) (setf (aref  nuevo-estado 2) 0)) ;Sale de un puente 
            ((AND (OR (eql etiqueta-op :Derecha) (eql etiqueta-op :Izquierda))  ( =  new-wall 16) )     (setf (aref  nuevo-estado 2) 1))     ;Entra a un puente
            ((AND (OR (eql etiqueta-op :Derecha) (eql etiqueta-op :Izquierda))  ( =  current-wall 16) ) (setf (aref  nuevo-estado 2) 0)) ;Sale de un puente 
            (T (setf (aref  nuevo-estado 2) level));Todo lo demás
        )
        nuevo-estado
    )
)
...
(defun add-dimension (estado)
    (let* ( (x  (aref  estado 0))                         
            (y  (aref  estado 1))
            (nuevo-estado  (make-array 3))
        )
        (setf (aref  nuevo-estado 0) x (aref  nuevo-estado 1) y (aref  nuevo-estado 2) 0 ) 
        nuevo-estado
    )
)
...
(defun a-estrella ()
    (setq *start* (add-dimension *start*)  )
    (setq *goal* (add-dimension *goal*)  )
